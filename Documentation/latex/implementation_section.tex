...
\begin{figure}[t]
    ...
    \begin{tikzpicture}[
        ...
        \node[midbox, text width=6.2cm, font=\scriptsize] (helpers) at (12.9,-4.2) {\texttt{HandDwellProgressBar.cs}\\\texttt{RoundedRectGraphic.cs}\\\texttt{DepthQueGradient.cs}};
...
Although the runtime is centered on the runner and the effect layer, three smaller support scripts are still important for the participant-facing behaviour of the prototype. \texttt{HandDwellProgressBar.cs} drives the dwell-confirmation and transition progress visualization, so the participant can see when a dwell response has been accepted and when the next trial is loading. \texttt{RoundedRectGraphic.cs} generates the rounded UI geometry used by that progress indicator. \texttt{DepthQueGradient.cs} colors the scene's depth-cue stripes to make forward distance toward the target wall easier to perceive. These scripts are smaller in scope, but they are part of how the designed interaction is made readable during use.

Figure~\ref{fig:unity-embodied-loading} makes this support layer visible in the actual task view. In this screenshot, the participant is represented through the embodied hand and pointing ray rather than through a controller model, while the horizontal bar above the targets shows the same progress-display system used for dwell confirmation and task transitions. The image is therefore important because it shows that input mode is not only an internal configuration in the runner: it changes the visible embodiment while the progress cue remains available as a stable part of the interaction language.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.88\linewidth]{img/3_Implementation/EmbodyDefaultWithLoading.png}
    \caption{Embodied interaction with the progress bar visible. The screenshot shows the participant-facing exposure view in hand-tracking mode: the embodied hand and ray replace the controller model, the active target still carries the bullseye cue, and the horizontal bar above the target wall visualizes either dwell buildup or the short transition/loading interval between accepted responses. This makes the embodiment change and the participant-facing confirmation/loading cue visible in one image.}
...
\begin{figure}[H]
    ...
    \end{minipage}
    \caption{Open-Loop Pointing in Unity. Left: the participant-facing stereoscopic view, which keeps the visual presentation sparse and concentrates attention on the target wall, the pointing ray, and the status readout. Right: an oblique scene view of the same task, which makes the wall-based pointing geometry easier to inspect from outside the headset view. These two screenshots together show that Open-Loop Pointing is implemented as the simplest wall-directed measurement task rather than as a complex scene interaction.}
...
Exposure is where the runtime-side effect variations become most visible, because the repeated target-acquisition structure stays the same while the perturbation changes. Translation applies a fixed world-space offset to the pose and ray. Rotation keeps the pose origin but rotates the pointing direction around the chosen axis. Skew is the most prism-like of the three in visual terms: it shifts the visible world and pointer representation while the scoring ray remains unchanged. The screenshots in Figure~\ref{fig:unity-exposure-effect-views} therefore matter because they show the same exposure grammar under different perturbation classes rather than four unrelated scenes.
...
\begin{figure}[H]
    ...
    \caption{Exposure example from the Quality tab. Each point is one exposure attempt, colored by hit or miss, and the vertical value is the final distance from target center on the board. The cards summarize how many attempts were included, the average absolute error, how many attempts exceeded the outlier threshold, whether error drifted over time, and the hit rate for the block. This view is useful for deciding whether an exposure block looked stable enough to support later interpretation.}
...
\begin{figure}[H]
    ...
    \end{minipage}
    \caption{Measurement-task examples from the Spatial tab. Left: LineBisection baseline responses plotted as signed deviation from the true midpoint, with the cross marking the midpoint of the displayed line. Right: OpenLoop baseline responses plotted as signed deviation from the intended target center, again with the cross marking the ideal centre. The purpose of this view is to expose spatial bias directly rather than only through aggregate trial statistics.}
    \label{fig:rshiny-spatial-measurement}
\end{figure}

\subsubsection{Trajectory View}

The Trajectories tab is the most detailed view in the dashboard, but it deliberately behaves differently for Exposure and for the measurement tasks. Exposure produces a dense sequence of samples within each target-acquisition attempt, so the tab can reconstruct the projected approach path on the target plane, break the visible response window into acceleration, deceleration, and hover, and then inspect how final error evolves across attempts. OpenLoop and Line Bisection do not always benefit from the same path view, so for those tasks the tab instead becomes a trial-by-trial inspection tool that overlays baseline and post and shows both signed response values and absolute error.
