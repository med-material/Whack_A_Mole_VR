...
extract_phase_profile <- function(meta_data, sample_data, event_data, selected_task = "Exposure", selected_block = "Exposure", selected_attempt = NA_real_, time_window = 1.0) {
  ...
  if (selected_task == "Exposure") {
    ...
    start_event <- event_data |>
      ...
      mutate(
        ...
        target_radius = as_num(TargetRadiusMeters)
      ) |>
  ...
  path <- path |>
    mutate(
      ...
      speed_mps = sqrt(dx^2 + dy^2) / pmax(dt, 1e-4),
      speed_mps = ifelse(is.finite(speed_mps), speed_mps, NA_real_),
      speed_mps = replace_na(speed_mps, 0),
      distance_to_target = sqrt(relative_x^2 + relative_y^2)
    )

  peak_idx <- which.max(path$speed_mps)
  if (length(peak_idx) == 0 || !is.finite(path$speed_mps[[peak_idx]])) {
    return(list(error = "Peak speed could not be estimated for this response"))
  }

  peak_speed <- path$speed_mps[[peak_idx]]
  speed_threshold <- max(0.02, peak_speed * 0.2)
  near_target <- path$distance_to_target <= target_radius
  slow_enough <- path$speed_mps <= speed_threshold
...
build_phase_plot <- function(meta_data, sample_data, event_data, selected_task = "Exposure", selected_block = "Exposure", selected_attempt = NA_real_, time_window = 1.0) {
  ...
  phase_plot <- plot_ly(
    ...
    color = ~Phase,
    colors = phase_colors,
    text = ~paste0(Phase, "<br>", round(DurationMs), " ms"),
    textposition = "inside",
    hovertemplate = paste0(
      "<b>%{customdata}</b><br>",
      "Duration: %{x:.0f} ms<extra></extra>"
    ),
    customdata = ~as.character(Phase)
  ) |>
    layout(
      barmode = "stack",
      showlegend = TRUE,
  ...
  speed_plot <- plot_ly(
    ...
    hovertemplate = paste0(
      "Time: %{x:.0f} ms<br>",
      "Projected aim speed: %{y:.3f} m/s<extra></extra>"
    ),
    name = "Projected aim speed"
  ) |>
    add_segments(
      x = min(profile$speed_df$TimeMs, na.rm = TRUE),
      xend = max(profile$speed_df$TimeMs, na.rm = TRUE),
      y = profile$speed_threshold_mps,
      yend = profile$speed_threshold_mps,
      line = list(color = "#16a34a", dash = "dash"),
      inherit = FALSE,
      hoverinfo = "text",
      text = sprintf("Hover speed threshold: %.3f m/s", profile$speed_threshold_mps),
      showlegend = FALSE
    ) |>
    add_markers(
      ...
      name = "Peak speed",
      hovertemplate = "Peak speed<extra></extra>",
      showlegend = FALSE
    ) |>
    layout(
      xaxis = list(title = "Time within visible response window (ms)"),
      yaxis = list(title = "Projected aim speed (m/s)", zeroline = FALSE),
      showlegend = FALSE
    )
  ...
  if (!is.na(profile$hover_start_ms)) {
    speed_plot <- speed_plot |>
      add_segments(
        x = profile$hover_start_ms,
        xend = profile$hover_start_ms,
        y = 0,
        yend = max(profile$speed_df$SpeedMps, na.rm = TRUE),
        line = list(color = "#16a34a", dash = "dot"),
        inherit = FALSE,
        hoverinfo = "text",
        text = "Hover phase begins",
        showlegend = FALSE
      )
  }
